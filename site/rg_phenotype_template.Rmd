...
## Sex-stratified sections removed
...
create_volcano_plot <- function(dat_pheno, params)
{
	...
	if (file.exists("../inputs/Fields_and_categories.tsv")) {
		...
		df_merge <- copy(dat_pheno)
		df_merge$level2cat <- "Trait"
		df_merge$finallevelcat <- "Trait"
	}
...
create_table <- function(dat_pheno, params, phenotypic_corr=FALSE)
{
	...
	# Use local phenotype IDs and summary pages
	  	dat_pheno$p2_site <- dat_pheno$p2
...
if (both_sexes_defined)
	create_table(dat_pheno, params, phenotypic_corr=FALSE)
```
...
<!-- Sex-stratified sections removed for FinnGen single-cohort results -->
